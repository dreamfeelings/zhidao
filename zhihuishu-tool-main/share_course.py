...
for c_index in range(start - 1, end):
    ...
    plan_time = 25
    # 遍历该课程的视频
    print(f'一共有{len(video_lesson_list)}节课\n\n')
    ...
    for vl in video_lesson_list:
        ...
        if local_study_time <= plan_time * 60 or model == 2:
            ...
            learning_note = course.pre_learning_note(ccCourseId=course_id, chapterId=vl.chapter_id,
                                                     ...
                                                     recruitId=recruit_id, videoId=vl.video_id,
                                                     small_lesson_id=vl.small_lesson_id)
            # 获取learn token id
            ...
            for i in range(int((vl.video_sec - 1) / 300) + 1 + len(question_times)):
                ...
                if local_study_time <= plan_time * 60:
                    ...
                    if question_times and qi <= len(question_times) and t_ - 300 < question_times[qi] <= t_:
                        ...
                        submit_study_record(vl, learn_time, dt, recruit_id, study_total_time, learning_token_id,
                                            course_id)
                        learn_time += dt
                        study_total_time += dt
