...
# cookie_name = input('请输入cookies名字：')
...
class Course(Account):
    ...
    def save_database_interval_time(self, watchPoint, sdsew, learningTokenId, courseId):
        """
        提交视频学习进度
        """
        ...
        #            f'"dateFormate":{int(round(time.time()) * 1000)}}}'
        raw_data = f'{{"ewssw":"{watchPoint}","sdsew":"{sdsew}","zwsds":"{ltid}","courseId":{courseId},' \
                   f'"dateFormate":{int(round(time.time()) * 1000)}}}'
